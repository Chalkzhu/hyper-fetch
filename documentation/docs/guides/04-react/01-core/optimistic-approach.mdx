...
function UserTableRow({ user }) {
  ...
  const { submit, onSubmitRequestStart, onSubmitError, onSubmitSuccess, refetch } = useSubmit(
  ...
  onSubmitSuccess(() => {
    // When it's successful, just refetch users list to pull new page data
    refetch(new RegExp("/users"));
  });
